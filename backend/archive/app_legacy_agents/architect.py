...
class ArchitectAgent(BaseAgent):
    ...
    async def process(self, context: AgentContext) -> InferenceOutput:
        ...
        if target_action == "configure_interview":
            ...
            prompt += "\n\nCRITICAL: Respond in JSON format: " + '{"thought": "...", "action_type": "configure_interview", "action_data": {"confidence_score": 0.0-1.0, "required_skills": [...], "milestones": [...], "estimated_duration": 45, "focus_areas": [...]}}'
        else:
            prompt = self._build_design_prompt(context)
            prompt += "\n\nCRITICAL: Respond in JSON format: " + '{"thought": "...", "action_type": "design_trajectory", "action_data": {"confidence_score": 0.0-1.0, "questions": [...]}}'
    ...
    def _build_design_prompt(self, context: AgentContext) -> str:
        ...
        return f"You are the Architect Agent. Design the technical trajectory.\nJD:\n{jd}\nResume:\n{resume}"
